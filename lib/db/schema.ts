...
export const emailPreferences = sqliteTable(
  "email_preferences",
  {
    ...
    monthlyReportEnabled: integer("monthly_report_enabled", { mode: "boolean" })
      .notNull()
      .default(true),
    monthlyReportDay: integer("monthly_report_day").notNull().default(1),
    lastMonthlyEmailSentAt: integer("last_monthly_email_sent_at", {
      mode: "timestamp",
    }),
    createdAt: integer("created_at", { mode: "timestamp" })
      .notNull()
...
export const emailQueue = sqliteTable(
  ...
  (table) => [
    index("email_queue_userId_idx").on(table.userId),
    index("email_queue_status_idx").on(table.status),
    index("email_queue_emailType_idx").on(table.emailType),
  ]
);
...
export const emailDeliveryLogs = sqliteTable(
  ...
  (table) => [
    ...
    index("email_delivery_logs_emailQueueId_idx").on(table.emailQueueId),
    index("email_delivery_logs_status_idx").on(table.status),
  ]
);

export const userMonthlyEmailSent = sqliteTable(
  ...
  (table) => [
    index("user_monthly_email_sent_userId_idx").on(table.userId),
    index("user_monthly_email_sent_calendarId_idx").on(table.calendarId),
    index("user_monthly_email_sent_year_month_idx").on(table.year, table.month),
  ]
);
...
export type UserMonthlyEmailSent = typeof userMonthlyEmailSent.$inferSelect;
export type NewUserMonthlyEmailSent = typeof userMonthlyEmailSent.$inferInsert;
...
export const emailDeliveryLogsRelations = relations(
  ...
  ({ one }) => ({
    emailQueue: one(emailQueue, {
      fields: [emailDeliveryLogs.emailQueueId],
...
export const userMonthlyEmailSentRelations = relations(
  userMonthlyEmailSent,
  ({ one }) => ({
    user: one(user, {
      fields: [userMonthlyEmailSent.userId],
      references: [user.id],
    }),
    calendar: one(calendars, {
      fields: [userMonthlyEmailSent.calendarId],
      references: [calendars.id],
    }),
  })
);
